...
goal_x = 0.700344  # Default value
goal_y = 4.908462  # Default value
linear_velocity = 1.5  # Default value
angular_velocity = 0.5  # Default value
target_angle = 0.0
cmd_vel_pub = None
...
def processData():
    ...
    if robot_position_current != None and (clock_last == 0.0 or clock_current - clock_last >= 0.1):
        ...
        if distance_to_goal>0.1:
            ...
            if agent_positions_current != None and target_position == None:
                for i in range(len(agent_positions_current.name)):
                    if 'actor' in agent_positions_current.name[i] and agent_positions_current.pose[i].position.z > 0:
                        ...
                        curren_agent = Agent(agent_positions_current.pose[i].position.x, agent_positions_current.pose[i].position.y, current_goal_x, current_goal_y, 0.3, current_speed, yaw, NonCooperativePolicy, UnicycleDynamics, [OtherAgentsStatesSensor], i)
            ...
            if len(agents_array) >1 :
                ...
                #collision_avoidance_env.collision_dist = 0.45
...
def clock_callback(msg):
    ...
    if robot_position_current!= None  and (clock_pub_last == 0.0 or clock_current - clock_pub_last >= 0.1):
        distance_to_goal = math.sqrt((goal_x-robot_position_current.position.x)**2 + (goal_y-robot_position_current.position.y)**2)
